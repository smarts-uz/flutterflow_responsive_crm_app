...
serve(async (req) => {
  ...
  try{
    ...
    const { first_name, last_name, model, color, car_number} = await req.json()

    const {data, error} = await supabaseClient.from('drivers')
        .insert({ first_name: first_name, last_name: last_name, model: model, color: color, car_number: car_number })
        .select('*')
